...
$("#globalSearchForm").onsubmit = (e) => {
  ...
  if (codeMatch) {
    ...
    if (found) {
      openMapIncident(found.id);
      toast(`Encontrado con BST: ${found.codigo}`);
      return;
    }
  }
...
renderMainMap = async function () {
  ...
  if (data.settings.showRoutes) {
    let n = 0;
    for (const incident of visibleIncidents.filter(
      ...
      if (route.geometry) {
        addRouteLayer(
          ...
          `op-route-${n++}`,
          route.geometry,
          routeColorByCategory(incident.categoria),
          6,
          0.96,
        );
      }
...
drawRouteComparison = async function (unit, incident) {
  ...
  const im = makeMarker(
    incident.categoria,
    incidentSymbol(incident.categoria),
    incident.codigo,
  );

  routeMarkers.push(
    new maplibregl.Marker({ element: um, anchor: "center" })
      .setLngLat([unit.lng, unit.lat])
      .addTo(routeMap),
  );
  routeMarkers.push(
    new maplibregl.Marker({ element: im, anchor: "center" })
  ...
  const palette = ["#39d17e", "#38a8ff", "#b764ff", "#ffffff"].filter(
  ...
  $("#routeAlternatives").innerHTML =
    `<div class="route-color-legend">
      ...
      ${routes
        ...
        .map(
          (r, n) =>
            `<span><i style="background:${palette[n]}"></i>Alternativa ${n + 2}</span>`,
        )
        .join("")}
    </div>` +
    routes
...
renderReports = function () {
  ...
  const cards = [
    [
      ...
      `${avg} min`,
      "Desde asignación hasta resolución en casos con dato",
    ],
  ...
  $("#typeBars").innerHTML = Object.entries(cats)
    .map(
      ([cat, count]) => `
    <div class="bar-row">
      <span>${names[cat]}</span>
...
renderStructure = function () {
  ...
  const purpose = {
    arreglo:
      "Administra la flota real. Las pantallas de Unidades y despacho leen y modifican este arreglo.",
    lista:
      "Guarda el historial real. Cada movimiento de un incidente se inserta como un nodo y se conserva en orden.",
    ...
    arbol:
      "Funciona como índice de incidentes por código. El buscador global usa el BST cuando escribes INC-204 o 204.",
    grafo:
      "Sirve como respaldo lógico entre sectores. Si no hay servicio vial, BFS indica el corredor de sectores que conecta la unidad con el incidente.",
  };
  ...
  const operationHelp = {
    arreglo: {
      "agregar()": "registra una nueva unidad",
      "buscar()": "encuentra una unidad por código",
      "modificar()": "actualiza sus datos",
      "eliminar()": "retira una unidad",
      "recorrer()": "lee toda la flota",
    },
    lista: {
      "insertar()": "agrega un evento al historial",
      "buscar()": "encuentra un movimiento",
      "modificar()": "cambia un nodo",
      "eliminar()": "quita un nodo",
      "recorrer()": "lee la cronología completa",
    },
    pila: {
      "apilar()": "guarda una acción",
      "desapilar()": "saca la última para deshacer",
      "cima()": "muestra qué se desharía",
      "estaVacia()": "comprueba si hay acciones",
    },
    cola: {
      "encolar()": "pone un incidente al final",
      "desencolar()": "retira el frente",
      "frente()": "muestra el siguiente",
      "estaVacia()": "comprueba si quedan pendientes",
    },
  }[selectedStructure];
...
drawTree = function () {
  ...
  function rec(node, x, y, spread) {
    ...
    if (node.izquierda) {
      ...
      rec(node.izquierda, x - spread, ny, spread * 0.55);
    }

    if (node.derecha) {
      drawLine(svg, x, y, x + spread, ny);
      rec(node.derecha, x + spread, ny, spread * 0.55);
    }
